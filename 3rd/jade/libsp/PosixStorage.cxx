...
# define WIN32_UNICODE_FILENAMES
#endif 
 
#if defined(SP_WIDE_SYSTEM) || defined(WIN32_UNICODE_FILENAMES)
#include <windows.h>
#endif
...
#if defined(SP_WIDE_SYSTEM) || defined(UNICODE_FILENAMES)
...
static bool starts_with(const StringC& id, const char* pfx, unsigned len)
{
    if (id.size() <= len)
        return false;
    for (unsigned i = 0; i < len; ++i)
        if (id[i] != pfx[i] && id[i] != toupper(pfx[i]))
            return false;
    return true;
}

Boolean 
PosixStorageManager::guessIsId(const StringC &id, const CharsetInfo &) const
{
    if (isAbsolute(id))
        return 1;
    if (starts_with(id, &FILE_PFX[0], sizeof(FILE_PFX) - 1))
        return 1;
#if defined(SP_WIDE_SYSTEM) || defined(UNICODE_FILENAMES)
    String<FChar> cfilename;
    cfilename.resize(id.size());
    for (size_t i = 0; i < id.size(); ++i) 
        cfilename[i] = id[i];
    cfilename += FChar(0);
#else
    String<FChar> cfilename = filenameCodingSystem_->convertOut(id);
#endif
    int fd = openFile(cfilename.data());
    if (fd > 0) {
        close(fd);
        return 1;
    }
...
StorageObject *
PosixStorageManager::makeStorageObject(const StringC &ospec,
				       const StringC &base,
				       Boolean search,
				       Boolean mayRewind,
				       Messenger &mgr,
				       StringC &found)
{
  if (ospec.size() == 0) {
    mgr.message(PosixStorageMessages::invalidFilename,
		StringMessageArg(ospec));
    return 0;
  }
  StringC spec;
//  if (ospec.size() > 5 && ospec[0] == 'f' && ospec[1] == 'i' &&
//      ospec[2] == 'l' && ospec[3] == 'e' && ospec[4] == ':') { 
  const unsigned PFX_SZ = sizeof(FILE_PFX) - 1;
  if (starts_with(ospec, &FILE_PFX[0], PFX_SZ)) {
      bool seen_only_slash = '/' == ospec[PFX_SZ];
      for (size_t i = PFX_SZ; i < ospec.size(); ++i) {
          if (seen_only_slash) {
              if (ospec[i] == '/')
                  continue;
#if defined(_WIN32)
              if (i + 1 < ospec.size() - 1 && ospec[i + 1] != ':')
#endif
              spec += '/';
              seen_only_slash = false;
          }
          spec += ospec[i];
      }
  } else
...
#if defined(SP_WIDE_SYSTEM) || defined(UNICODE_FILENAMES)
    String<FChar> cfilename;
    cfilename.resize(filename.size());
