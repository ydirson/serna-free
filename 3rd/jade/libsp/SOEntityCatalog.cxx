...
private:
  ...
  const ExternalIdResolver* resolver_;
};

class SOEntityCatalog : public EntityCatalog {
public:
  SOEntityCatalog(Ptr<ExtendEntityManager> em, const ExternalIdResolver* resolver);
  ...
  /*! Syntext extention to OSEntityCatalog*/

    List<StringC>*
    makeEntryList(const CharsetInfo &charset,
                  const Char& space,
                  const EntityCatalog::PublicIdPredicate& predicate) const;
private:
  ...
  public:
    ...
    /*! Syntext extention to Table*/
    const HashTable<StringC, CatalogEntry>& normalEntries() const;
    /*! Syntext extention to Table*/
    const HashTable<StringC, CatalogEntry>& overrideEntries() const;
  private:
					 ...
					 Messenger &mgr) const
{
  ...
  while (maps.size() > 0) {
    ...
    SOEntityCatalog *catalog = new SOEntityCatalog(em, resolver_);
...
SOCatalogManagerImpl::makeCatalog(StringC &systemId,
				  ...
				  Messenger &mgr) const
{
  SOEntityCatalog *entityCatalog = new SOEntityCatalog(em, resolver_);
  CatalogParser parser(*catalogCharset_);
  size_t i;
...
SOEntityCatalog::SOEntityCatalog(Ptr<ExtendEntityManager> em, const ExternalIdResolver* resolver)
: em_(em), catalogNumber_(0), haveSgmlDecl_(0), haveDocument_(0),
  haveCurrentBase_(0), resolver_(resolver)
{
}
...
static bool has_file_scheme(const StringC& s)
{
    static const char FS[] = "file://";
    if (s.size() < sizeof(FS))
        return false;
    for (unsigned i = 0; i < sizeof(FS) - 1; ++i)
        if (tolower(s[i]) != FS[i])
            return false;
    return true;
}
...
{
  if (resolver_) {
      if (const StringC* s = entity.systemIdPointer()) {
          if (!has_file_scheme(*s)) {
              em_->expandSystemId(*s,
                                  entity.
                                  defLocation(),
      ...
      bool rv = resolver_->resolveExternalId(*entity.publicIdPointer(),
                                             *entity.systemIdPointer(),
                                             result);
      if (rv)
          return rv;
  }
  ...
  Boolean rv = 0;
  if (entry)
    rv = expandCatalogSystemId(entry->to,
				 entry->loc,
				 entry->baseNumber,
				 entity.dataType() == EntityDecl::ndata,
				 charset,
				 ...
				 result);
  if (entity.systemIdPointer())
    rv = em_->expandSystemId(*entity.systemIdPointer(),
        		       entity.
        		       defLocation(),
        		       entity.dataType() == EntityDecl::ndata,
        		       charset,
...
// Syntext extentions to SP:

/*! Returns list of all catalog pubids not contradicting predicate

It checks the pubid list but does not take into account the delegates
*/

List<StringC>*
SOEntityCatalog::makeEntryList(const CharsetInfo &charset,
                               const Char& space,
                               const EntityCatalog::PublicIdPredicate& predicate) const
{
    List<StringC>* list = new List<StringC>;

    const MessageType1* msg = 0;

    HashTableIter<StringC,CatalogEntry>
        iter1(publicIds_.overrideEntries());
    HashTableIter<StringC,CatalogEntry>
        iter2(publicIds_.normalEntries());

    HashTableIter<StringC,CatalogEntry> *iters[2];
    int nIter = 0;
    iters[nIter++] = &iter1;
//    if (!overrideOnly) lets not be obscurely generic
        iters[nIter++] = &iter2;
    for (int i = 0; i < nIter; i++) {
        HashTableIter<StringC,CatalogEntry> &iter = *iters[i];
        const StringC *tem_stringc;
        const CatalogEntry *tem_ce;
        while (iter.next(tem_stringc, tem_ce)) {
            Text text;
            text.addChars(*tem_stringc, Location());
            PublicId pubid;
            if (!pubid.init(text, charset, space, msg))
                continue; // Skip if parsing is wrong

            if (predicate.isTc) {
                PublicId::TextClass tc;
                Boolean result = pubid.getTextClass(tc);
                if (!result || (predicate.tc_ != tc))
                    continue;
            }
            if (predicate.isOt) {
                PublicId::OwnerType ot;
                Boolean result = pubid.getOwnerType(ot);
                if (!result || (predicate.ot_ != ot))
                    continue;
            }
            if (predicate.isOwner) {
                StringC owner;
                Boolean result = pubid.getOwner(owner);
                if (!result || (predicate.owner_ != owner))
                    continue;
            }
            if (predicate.isUnavailable) {
                Boolean unavailable;
                Boolean result = pubid.getUnavailable(unavailable);
                if (!result || (predicate.unavailable_ != unavailable))
                    continue;
            }
            if (predicate.isDescription) {
                StringC description;
                Boolean result = pubid.getDescription(description);
                if (!result || (predicate.description_ != description))
                    continue;
            }
            if (predicate.isLang) {
                StringC lang;
                Boolean result = pubid.getLanguage(lang);
                if (!result || (predicate.lang_ != lang))
                    continue;
            }
            if (predicate.isDs) {
                StringC ds;
                Boolean result = pubid.getDesignatingSequence(ds);
                if (!result || (predicate.ds_ != ds))
                    continue;
            }
            if (predicate.isDv) {
                StringC dv;
                Boolean result = pubid.getDisplayVersion(dv);
                if (!result || (predicate.dv_ != dv))
                    continue;
            }
            list->append(*tem_stringc);
        }
    }
...
const HashTable<StringC, CatalogEntry>&
SOEntityCatalog::Table::normalEntries() const
{
    return normalEntries_;
}

const HashTable<StringC, CatalogEntry>&
SOEntityCatalog::Table::overrideEntries() const
{
    return overrideEntries_;
}
