...
Ui_InsertAddressDialog, DialogBase = \
    load_qt_uitype(__file__, "InsertAddressDialog2.ui")

class InsertAddressDialog(DialogBase, Ui_InsertAddressDialog):
    def __init__(self, *args):
        DialogBase.__init__(self, *args)
        self.setupUi(self)

class InsertAddressItem(UiLiquidItemBase):
    def __init__(self):
        UiLiquidItemBase.__init__(self)

    def makeWidget(self, parent, type):
        return InsertAddressDialog(parent)

    def itemClass(self):
        return "InsertAddress"

class InsertAddressItemMaker(UiItemMaker):
    def __init__(self):
        UiItemMaker.__init__(self)

    def makeItem(self, action, properties):
        return UiItemMaker.makeLiquidItem(properties, InsertAddressItem())
...
class InsertAddress(DocumentPlugin):
    ...
    def __init__(self, a1, a2):
        ...
        self.sernaDoc().addItemMaker("InsertAddress", InsertAddressItemMaker()) 
    ...
    def showDialog(self):
        ...
        item = self.sernaDoc().findItemByClass("InsertAddress")
        if item:
           item.setVisible(True)
           return
        props = PropertyNode("")
        props.makeDescendant("name").setString("InsertAddress")
        props.makeDescendant("inscription").setString("Insert Address")
        item = self.sernaDoc().makeAndShow("InsertAddress", props)
