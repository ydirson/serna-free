...
<xsl:template match="element | *[contains(@class,' elementref/element ')]">
  ...
  <fo:table xsl:use-attribute-sets="table.data frameall">
    ...
    <fo:table-row>
      ...
      <fo:table-cell start-indent="2pt" background-color="silver" padding="2pt" text-align=
        ...
        <fo:block>
        <xsl:call-template name="getString">
          <xsl:with-param name="stringName" select="'Default Value'"/>
          <xsl:with-param name="stringFile" select="$ERStringFile"/>
        </xsl:call-template>
        </fo:block>
      </fo:table-cell>
...
<xsl:template match="*[contains(@class,' xelementref/xattribute ')]">
  <xsl:apply-templates/>
</xsl:template>

<!-- For authoring, support all attlist attrs (for print, only attname, attdesc and attrequired) -->
...
<xsl:template match="*[contains(@class,' elementref/atttype ')]">
  <fo:table-cell column-number="3" start-indent="2pt" background-color="white" padding="2pt" xsl:use-attribute-sets="frameall">
    <fo:block><xsl:apply-templates/></fo:block>
  </fo:table-cell>
</xsl:template>

<xsl:template match="*[contains(@class,' elementref/attdefvalue ')]">
  <fo:table-cell column-number="4" start-indent="2pt" background-color="white" padding="2pt" xsl:use-attribute-sets="frameall">
    <fo:block><xsl:apply-templates/></fo:block>
  </fo:table-cell>
</xsl:template>

<xsl:template match="*[contains(@class,' elementref/attrequired ')]">
  <fo:table-cell column-number="5" start-indent="2pt" background-color="white" padding="2pt" xsl:use-attribute-sets="frameall">
    <fo:block><xsl:apply-templates/></fo:block>
  </fo:table-cell>
</xsl:template>

<!-- which means we need to consume these events to prevent fallthrough (nulled out for Serna) -->
<xsl:template match="*[contains(@class,' elementref/xatttype ')]|*[contains(@class,' elementref/xattdefvalue ')]">
