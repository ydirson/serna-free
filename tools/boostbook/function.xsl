...
<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform"
  ...
  <xsl:template name="function-parameters">
    ...
    <xsl:if test="$parameters">
      ...
      <xsl:variable name="text" select="concat($type-value, $name, $default)"/>
  ...
  <xsl:template name="function.documentation.compact">
    ...
    <xsl:choose>
      <xsl:when test="count(ancestor::free-function-group) &gt; 0
        ...
        <listitem><xsl:copy-of select="$text"/><cmdsynopsis><sbr/></cmdsynopsis></listitem>
      </xsl:when>
      <xsl:otherwise>
...
-->
  ...
  <xsl:template name="function-requirements">
    ...
    <xsl:if test="purpose">
      <para><xsl:apply-templates select="purpose"/></para>
    </xsl:if>
