...
<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform"
  ...
  <xsl:param name="is-python" select="true()"/>
  <xsl:include href="reference.xsl"/>

  <xsl:output method="xml" encoding="UTF-8"/>
<!--ego 5.2.2.3.a The docbook output is now generated with PUBID. 
        This should be removed, otherwise the document could not be inserted as external entity
  ...
  <xsl:param name="boost.root" select="'../../sapi'"/>
  ...
  <xsl:template match="library-reference">
    <xsl:choose>
      ...
      <xsl:otherwise>
        <section>
          <xsl:choose>
            <xsl:when test="$is-python">
             <title>Serna Python API Reference Guide</title>
            </xsl:when>
            <xsl:otherwise>
             <title>Serna C++ API Reference Guide</title>
            </xsl:otherwise>
          </xsl:choose>
  ...
  <xsl:template match="header">
    <xsl:if test="*">
      <section>
        ...
        <title>
          <ulink>
            ...
            <xsl:attribute name="url">
              <xsl:value-of select="$qurl"/>
              <xsl:if test="$is-python">              
          ...
          <xsl:choose>
            <xsl:when test="$is-python">
              ...
              <xsl:value-of select="translate(substring-before(@name,'.h') , '/','.')"/>
              <xsl:text>)</xsl:text>
            </xsl:when>
            <xsl:otherwise>
        ...
        <!--egoxsl:if test="namespace|class|struct|union">
          <xsl:call-template name="synopsis">
            <xsl:with-param name="text">
