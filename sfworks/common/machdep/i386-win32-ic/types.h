...
// 
/*! Atomic types for i386/GCC
 */
#ifndef MACHDEP_I386_IC_TYPES_H_
#define MACHDEP_I386_IC_TYPES_H_
...
#define UINT64_VALUE(x) x##ui64
#define INT64_VALUE(x)  x##i64
...
#define _UNDEF_(t, uval) \
    ...
    _UNDEF_(int64,  0x8000000000000000ui64) \
    _UNDEF_(uint8,  0x80) \
    _UNDEF_(uint16, 0x8000) \
    _UNDEF_(uint32, 0x80000000L) \
    _UNDEF_(uint64, UINT64_VALUE(0x8000000000000000))
...
inline void set_undef(bool& v)            { (int8&) v = int8(0x80); }
inline void set_undef(bool* pv, uint32 n) { while (n--) (int8&) (*pv++) = int8(0x80); }
inline bool is_true(bool v)  { return !is_undef(v) && v;  }
inline bool is_false(bool v) { return !is_undef(v) && !v; }

#endif // MACHDEP_I386_IC_TYPES_H_
