...
void MmlRootBaseNode::paintSymbol(QPainter *p) const
{
    ...
    QRect radicalRect(QFontMetrics(fn).boundingRect(g_radical_char));
    p->setViewport(r);
    p->setWindow(radicalRect);
    p->setFont(font());
...
bool QtMmlDocument::setContent(const GroveLib::Node* node, QString *errorMsg)
{
    return m_doc->setContent(node, errorMsg);
}
