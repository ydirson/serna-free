...
#ifndef XPATH_STRING_VALUE_H
#define XPATH_STRING_VALUE_H
...
class NodeSet;
class StringValue;
typedef COMMON_NS::RefCntPtr<const StringValue> ConstStringValuePtr;


class XPATH_EXPIMP StringValue : public ValueImpl {
public:
    XPATH_OALLOC(StringValue);
    StringValue(const COMMON_NS::String& str);
    ...
    const COMMON_NS::String&       getString() const {return string_;}
    const NodeSet&      getNodeSet() const;

    void                dump() const;
    static const ConstValueImplPtr& empty();
...
private:
    const COMMON_NS::String        string_;
};

} // namespace Xpath

#endif // XPATH_STRING_VALUE_H
