...
#ifndef XPATH_SLASH_EXPR_H
#define XPATH_SLASH_EXPR_H
...
class XPATH_EXPIMP SlashExpr : public ExprImpl {
public:
    XPATH_OALLOC(SlashExpr);
    SlashExpr(ExprImplPtr left, ExprImplPtr right);

    const ExprImpl& left() const {return *left_;}
    const ExprImpl& right() const {return *right_;}

    Type            type() const {return SLASH;}
